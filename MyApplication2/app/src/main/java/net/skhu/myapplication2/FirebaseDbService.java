...
public class FirebaseDbService implements ChildEventListener {
    ...
    ArrayList<Item3> itemList; // RecyclerView에 표시할 데이터 목록
    DatabaseReference databaseReference;

    public FirebaseDbService(Context context, Firebase2Adapter recyclerView2Adapter, ArrayList<Item3> itemList) {
    ...
    public void addIntoServer(Item3 item) {
    ...
    public void updateServer(Item3 item) {
        // 서버 데이터를 update 한다.
    ...
    public void onChildAdded(DataSnapshot dataSnapshot, String previousChildName) {
        ...
        Item3 item = dataSnapshot.getValue(Item3.class);  // 새 데이터 항목을 꺼낸다.
...
//        recyclerView2Adapter.notifyDataSetChanged(); // RecyclerView를 다시 그린다.
    ...
    public void onChildChanged(DataSnapshot dataSnapshot, String previousChildName) {
        ...
        Item3 item = dataSnapshot.getValue(Item3.class); // 수정된 데이터 항목을 꺼낸다.
        if (key.equals(item.getKey()) == false)
            Log.e("로그", "key값 불일치 오류");
