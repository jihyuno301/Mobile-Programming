...
public class MainActivity extends AppCompatActivity {
    ...
    static final int RC_LOGIN = 1; // 로그인 액티비티 호출을 구별하기 위한 식별번호
    ...
    void startLoginActivity() {
        // 이메일 인증과 구글계정 인증을 사용하여 로그인 가능하도록 설정
        ...
        // 로그인(sign in) 액티비티를 호출
        // startActivityForResult 메소드는 다른 액티비티를 호출할 때 사용하는 메소드이다.
        // startActivityForResult = 결과를 받기 위해서 액티비티를 호출한다는 뜻
        // 어떤 액티비티를 호출하고, 그 액티비티 실행 결과를 전달받겠다는 뜻.
    ...
    void setUserName() {
        ...
        if(currentUser != null) // 로그인 성공했다면
            ...
            textView.setText("Anoymous");
        invalidateOptionsMenu(); // 메뉴 상태를 변경해야 함.
    }

    // 로그아웃
    void logout() {
        FirebaseAuth.getInstance().signOut();
        setUserName(); // 화면에 표시된 사용자 이름을 Anoymouse로 바꾼다.
    }
